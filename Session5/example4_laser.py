...
def laser(x,y):
    ...
    # when laser() is called set laser_ready to fire so the program knows that the player can't shoot anymore until laser is back in rest
    laser_ready = False
...
while running:
    ...
    if laser_Y <= 0:
        laser_Y = 550
        laser_ready = True
    # if laser_ready is false then shoot laser vertically
    if not(laser_ready):
        laser(laser_X, laser_Y)
